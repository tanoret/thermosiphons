...
- Revised the **cost model around the latest vendor quote**: the factory heat-pipe line is now anchored near the vendor benchmark, not fixed at exactly $100. The app adjusts each pipe for diameter, depth, package complexity, and quantity while keeping the estimate close to the vendor guidance.
- Moved the **cost-first estimate** to the top of the dashboard: installed estimate, cost per ft², estimated factory unit cost, factory heat-pipe subtotal, and recommended pipe count are visible before the technical tabs.
...
The cost model is intentionally transparent and budgetary. The hardware line now uses an editable vendor **benchmark** near $100 for a reference factory-manufactured sealed heat pipe. The actual unit cost in the BOM is design-adjusted for diameter, depth, package complexity, and quantity, so it stays close to the vendor guidance without making every pipe exactly $100. The installed estimate still includes:

- factory-manufactured sealed heat pipes at the editable $/pipe price;
- receiving QA, labeling, and protective handling;
- conductive grout/backfill;
- near-surface heat-spreader strip/manifold for high-output packages;
- installation labor;
...
## v6 vendor-calibrated cost update

This build revises the cost model based on vendor feedback that the factory heat pipe can be manufactured for approximately **$100 each**, while avoiding a rigid $100-per-pipe assumption. The app exposes a vendor benchmark input, then estimates the factory unit cost near that value using pipe diameter, depth, package complexity, and quantity. Cost remains visible at the top of the dashboard and the BOM clearly separates factory hardware from installation, thermal backfill, heat-spreader, electrical/controls, overhead, distribution, and contingency costs.
