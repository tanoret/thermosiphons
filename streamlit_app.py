...
with st.sidebar:
    ...
    if goal == "Reduce thermal cracking":
        ...
        target_default = 5
        target_help = "Target reduction in wet-freeze or freeze hours. Passive thermosyphons usually reduce severity more than they eliminate every freeze hour."
    ...
    fluid = st.selectbox(
        ...
        fluid_options,
        index=fluid_options.index("High-output CO2 + thermal grout + heat spreader") if "High-output CO2 + thermal grout + heat spreader" in fluid_options else 0,
        help="The high-output package expands the design search to larger diameters, closer spacing, deeper boreholes, thermal grout, and a near-surface heat spreader.",
    )
    max_depth_ft = st.slider("Maximum constructable depth (ft)", 4.0, 24.0, 16.0, 1.0)
...
with tab_assumptions:
    ...
    st.markdown(
        """
        **What the model does:** solves a 1D transient finite-difference driveway/base/soil heat equation with hourly air temperature, wind, humidity, solar radiation, rain/snow energy loads, and a linearized surface energy balance. The thermosyphon field is represented as a conservative one-way heat-transfer source/sink between a deep soil cell and a near-surface cell.
