...
def _initial_temperature_profile(site: Site, centers: np.ndarray, soil: SoilProfile) -> np.ndarray:
    annual_mean = site.annual_ground_mean_c
...
def _sky_temperature_C(air_C: float, rh_pct: float, ghi_W_m2: float) -> float:
    ...
    return float(air_C - 4.5 - night_clear_penalty)


def _surface_coefficients(weather_row: pd.Series, albedo: float, emissivity: float) -> tuple[float, float]:
    ...
    h_conv = 4.0 + 4.0 * np.sqrt(wind)
    tref_K = max(air + 273.15, 230.0)
    sigma = 5.670374419e-8
    h_rad = 4.0 * emissivity * sigma * tref_K**3
    sky_C = _sky_temperature_C(air, rh, ghi)
    h_total = h_conv + h_rad
    q_const = (1.0 - albedo) * ghi + h_conv * air + h_rad * sky_C
    return float(h_total), float(q_const)
...
def run_thermal_simulation(
    ...
    bottom_temp_C = site.annual_ground_mean_c + config.bottom_gradient_K_m * config.max_depth_m
    bottom_g = k[-1] / max(dz[-1] / 2.0, 1e-6)

    T = _initial_temperature_profile(site, centers, soil)
    n_hours = len(weather)
    ...
    for step in range(n_total):
        ...
        h_total, q_const = _surface_coefficients(row, albedo=albedo, emissivity=emissivity)
        diag[0] += h_total
        rhs[0] += q_const
        diag[-1] += bottom_g
        ...
        if step >= n_total - n_hours:
            ...
            surf[out_i] = T[0]
            hp_flux[out_i] = q_hp
            q_solar_abs[out_i] = (1.0 - albedo) * max(float(row.get("ghi_W_m2", 0.0)), 0.0)
